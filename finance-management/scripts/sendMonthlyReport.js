...
const run = async () => {
  ...
  await connectDB();
  console.log(`${dryRun ? 'Previewing' : 'Sending'} monthly reports as of ${asOf.toISOString()}${force ? ' (force)' : ''}${onlyUser ? ` for user ${onlyUser}` : ''}`);

  const sent = await sendDueReports(asOf, { dryRun, force, onlyUser });
