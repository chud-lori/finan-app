...
Narrative discipline: lines never bake in a formatted amount — they speak in percentages, counts, category names and month labels so they read correctly in any currency. Raw amounts ride only on `tiles`, which the frontend formats via `useFormatAmount()`. Each tile carries `{ key, label, value, format: 'currency'|'percent'|'number', tone, baseline?, count? }`.

Materiality and the low base effect: tiles no longer carry a bare `delta` percentage. They carry the raw `baseline` (last month's figure, the prior net-worth reading, the mover's previous total) and the recap publishes a `materialityFloor` — `helpers/materiality.js`, 2% of the larger of this month's and last month's spend. The frontend (`lib/materiality.js#describeChange`) renders the change in currency, shows the before figure beside it, and only adds a percentage when the baseline is positive and itself clears the floor — a percentage off a negative baseline (a shrinking debt) would carry the opposite sign to the money beside it, so it is withheld and only the money is shown; a change under the floor gets no badge at all, so "unchanged" costs no ink. The same floor gates `topMover`, which now picks the biggest **money** move rather than the biggest ratio — a category going from a near-zero baseline to an ordinary purchase is not the month's headline. A mover carried by a single transaction reports `count: 1`, and both the narrative and the tile say so instead of quoting a ratio. A tile whose natural base is not the month's spend carries its own `floor` (the net-worth tile is judged against net worth, since a month of spend is a rounding error against it) and the frontend reads `tile.floor ?? materialityFloor`.
...
Recurring price rises: `detectRecurring` has always produced `price_up` alerts (a charge whose latest amount is well above the median of the prior ones) and nothing rendered them. `computeRunway` now passes them through as `priceChanges[] = { merchant, from, to }` — money only, no percentage, no advice — filtered by `materialPriceChanges` to rises that clear 2% of the user's recurring monthly total (`helpers/materiality.js`). `components/PaydayRunway.js` lists them under the bills. This is a multi-month, multi-observation signal, which is exactly why it should outrank single-month ratios.
...
**What the report contains.** The email is assembled entirely from documents already on disk — no ledger scan, so the per-user cost stays flat as history grows. `buildReportPayload` reads the target `Snapshot`, the prior month's `Snapshot`, both `NetWorthSnapshot` readings, `User.streakDays`/`longestStreak` and `MLInsight.anomalyCount`, then composes five blocks:

| Block | Built by | Source |
|---|---|---|
| Headline + comparison chip | `buildHeadline`, `buildBadge` | current vs prior `Snapshot` |
| Money in / money out cards, savings rate | `buildMoneyCards`, `buildSummaryLines` | `Snapshot` + `getSavingsCategoryNames` |
| Where it went (top 5 + rolled-up tail, bars) | `buildCategories` | `Snapshot.byCategory` |
| Versus the month before | `buildComparison` | prior `Snapshot`, via `recap.js#topMover` |
| The month at a glance | `buildGlance` | `NetWorthSnapshot`, streak, `MLInsight` |
...
| Method | Path | Rate limit | Auth | Description |
|--------|------|-----------|------|-------------|
| GET | `/api/recommendations` | 20/min | ✓ | 1–5 personalised rule-based nudges; query: `?tz=IANA`. A nudge may carry `figures` — either `{ from, to, fromLabel, toLabel, count }` for a change or `{ amount, amountLabel }` for a single figure — as raw numbers the frontend formats; the server never formats money |
...
**Money leads, percentages follow.** A nudge never states a percentage change on
its own. Amount-driven nudges (category overspend, spending over income, surplus
sweep, seasonal set-aside) ship raw `figures` and let the frontend format them,
and they only fire when the amount clears the materiality floor in
`helpers/materiality.js` (2% of the larger of this month's spend and the 3-month
average). A category whose month rests on one transaction is reported as a
one-off `info` nudge, not a `warning` to trim.
