...
- The group filter needs a category → group map, fetched from `GET /api/category` only when a group filter is actually chosen. Until it lands the charts stay unfiltered — bucketing everything as `other` would flash a false empty state.
- Period **and** filters live in the URL (`tab`, `y`, `m`, `cat`, `grp`, `type`, `min`, `max`), so a view survives a refresh and can be bookmarked. `useSearchParams` requires the page to sit inside a `<Suspense>` boundary.
- Period comparison (`vs Last Month` / `vs My Average`) is hidden while a filter is active: the reference payload is server-aggregated and unfiltered, so the deltas would not line up.
