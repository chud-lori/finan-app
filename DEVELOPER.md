...
Materiality and the low base effect: tiles no longer carry a bare `delta` percentage. They carry the raw `baseline` (last month's figure, the prior net-worth reading, the mover's previous total) and the recap publishes one `materialityFloor` — `helpers/materiality.js`, 2% of the larger of this month's and last month's spend. The frontend (`lib/materiality.js#describeChange`) renders the change in currency, shows the before figure beside it, and only adds a percentage when the baseline itself clears the floor; a change under the floor gets no badge at all, so "unchanged" costs no ink. The same floor gates `topMover`, which now picks the biggest **money** move rather than the biggest ratio — a category going from a near-zero baseline to an ordinary purchase is not the month's headline. A mover carried by a single transaction reports `count: 1`, and both the narrative and the tile say so instead of quoting a ratio.
