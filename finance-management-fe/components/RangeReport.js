...
const PRESETS = [
  { label: 'Last 7 days',   range: () => { const e = new Date(), s = new Date(); s.setDate(e.getDate() - 6); return [fmt(s), fmt(e)]; } },
  { label: 'Last 30 days',  range: () => { const e = new Date(), s = new Date(); s.setDate(e.getDate() - 29); return [fmt(s), fmt(e)]; } },
  { label: 'This month',    range: () => { const n = new Date(); return [fmt(new Date(n.getFullYear(), n.getMonth(), 1)), fmt(n)]; } },
  { label: 'Last 3 months', range: () => { const e = new Date(), s = new Date(); s.setMonth(e.getMonth() - 3); s.setDate(1); return [fmt(s), fmt(e)]; } },
  { label: 'This year',     range: () => { const n = new Date(); return [fmt(new Date(n.getFullYear(), 0, 1)), fmt(n)]; } },
];
...
function CategoryBreakdown({ transactions }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-5">
      <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-200 mb-4">Expense breakdown</h3>
...
export default function RangeReport() {
  ...
  return (
    <div>
      <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">Any custom date range — income, expenses and a breakdown.</p>
          ...
          <button key={p.label} onClick={() => handlePreset(p)}
            className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${
              active === p.label
                ...
                : 'bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300 hover:border-teal-300 hover:text-teal-700'
            }`}>
            {p.label}
          </button>
        ))}
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-4">
              <p className="text-xs text-gray-500 dark:text-slate-400 mb-1">Income</p>
              <p className="text-base sm:text-lg font-bold text-emerald-600 tabular-nums">{formatAmount(result.income)}</p>
            </div>
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-4">
              <p className="text-xs text-gray-500 dark:text-slate-400 mb-1">Expense</p>
          ...
          <div className="flex flex-col lg:flex-row gap-5 items-stretch lg:items-start">
            ...
            <div className="flex-1 min-w-0 bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between gap-3 flex-wrap">
                <div>
                  <h2 className="font-semibold text-gray-900 dark:text-slate-100">Transactions</h2>
                  ...
                  {['all', 'income', 'expense'].map(f => (
                    <button key={f} onClick={() => setTypeFilter(f)}
                      className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all capitalize ${
                        typeFilter === f ? 'bg-white dark:bg-slate-700 text-teal-700 dark:text-teal-300 shadow-sm' : 'text-gray-400 hover:text-gray-600'
                      }`}>
                      {f}
                    </button>
                  ))}
              ...
              {txns.length === 0 ? (
                ...
                <>
                  <ul className="sm:hidden divide-y divide-gray-100 dark:divide-slate-800">
                    {txns.map((t) => {
                        ...
                        <li key={tid} className="flex items-center gap-3 px-4 py-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-baseline justify-between gap-2">
                              <span className="font-medium text-gray-900 dark:text-slate-100 text-sm truncate">{t.description}</span>
                  ...
                  <div className="hidden sm:block overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50 dark:bg-slate-800 text-gray-500 dark:text-slate-400 text-xs uppercase tracking-wider">
                      ...
                      <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                        {txns.map((t, i) => {
                          ...
                          const income = t.type === 'income';
                          return (
                            <tr key={tid} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                              <td className="px-5 py-3 text-gray-400">{i + 1}</td>
                              <td className="px-5 py-3 font-medium text-gray-900 dark:text-slate-100 max-w-xs truncate">{t.description}</td>
                              <td className="px-5 py-3">
                                <span className="px-2 py-0.5 rounded-md text-xs bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 capitalize whitespace-nowrap">
                              ...
                              <td className={`px-5 py-3 text-right font-semibold tabular-nums whitespace-nowrap ${income ? 'text-emerald-600' : 'text-rose-600'}`}>
                                <span title={income ? 'Income' : 'Expense'}>{income ? '+' : '−'}{formatAmount(t.amount)}</span>
                              </td>
